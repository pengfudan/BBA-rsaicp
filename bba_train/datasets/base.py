...
import numpy as np
import math
from .draw_gaussian import draw_umich_gaussian, draw_umich_binary, gaussian_radius, draw_ellipse_gaussian
...
class BaseDataset(data.Dataset):
    ...
    def generate_ground_truth(self, image, annotation):
        ...
        am = np.zeros((image_h, image_w), dtype=np.float32)
        ...
        # index = np.zeros((self.max_objs, 3), dtype=np.int64)
        ...
        for k in range(num_objs):
            ...
            # radius = min(bbox_w / 2, bbox_h / 2)
            radius = max(0, int(radius))
            ct = np.asarray([cen_x, cen_y], dtype=np.float32)
            ct_int = ct.astype(np.int32)
            # index[k] = np.asarray([annotation['cat'][k], ct_int[0], ct_int[1]], dtype=np.int64)
            draw_umich_gaussian(hm[annotation['cat'][k]], ct_int, radius)
            r = int(max(bbox_w * np.cos(theta*np.pi/180) + bbox_h * np.sin(theta*np.pi/180), bbox_w * np.sin(theta*np.pi/180) + bbox_h * np.cos(theta*np.pi/180)))
            draw_ellipse_gaussian(am, ct_int, r, bbox_w, bbox_h, theta)
        ...
        '''
        ret = {'input': image,
               'ct_left':np.array(ct_left),
               'ct_right':np.array(ct_right),
               'hm': hm,
               'reg_mask': reg_mask,
               'ind': ind,
               'wh': wh,
