...
class splitbase():
    ...
    def SplitSingle(self, name, rate, extent):
        ...
        for obj in objects:
            ...
            xmin = math.floor(min(obj['poly'][0], obj['poly'][2], obj['poly'][4], obj['poly'][6]))
            ymin = math.floor(min(obj['poly'][1], obj['poly'][3], obj['poly'][5], obj['poly'][7]))
            xmax = math.ceil(max(obj['poly'][0], obj['poly'][2], obj['poly'][4], obj['poly'][6]))
            ymax = math.ceil(max(obj['poly'][1], obj['poly'][3], obj['poly'][5], obj['poly'][7]))
            obj_rec_all.append([xmin, ymin, xmax, ymax])
            # print(obj)
        ...
        while (left < weight):
            ...
            while (up < height):
                ...
                # TODO
                for obj in obj_rec_all:
                    if obj[0] >= left and obj[2] <= right:
                        in_list.append(obj)
                    elif obj[0] <= right and obj[2] > right:
                        inout_list.append(obj)
                    elif obj[0] > right:
                        out_list.append(obj)
                # print(len(in_list)+len(inout_list)+len(out_list))
            ...
            if (left + self.subsize >= weight):
                ...
                minx = 4096
                if len(inout_list) > 0:
                    for obj in inout_list:
                        minx = min(minx, obj[0])
                elif len(out_list) > 0:
                    for obj in out_list:
                        minx = min(minx, obj[0])
                left = minx - 1  # -1 can be modified
...
def rm_background(path):
    image_path = os.path.join(path,'images')
    label_path = os.path.join(path,'labelTxt')
    for label in os.listdir(label_path):
        filename = os.path.join(label_path, label)
        if os.path.getsize(filename) == 0:
            os.remove(filename)
            imagename = os.path.join(image_path, os.path.splitext(label)[0] + '.png')
            os.remove(imagename)
...
if __name__ == '__main__':
    ...
    # origin_path = r'/data3/plin/rsaicp/train/'
    origin_path = r'/data3/plin/rsaicp_rotate/train/'
    # split_path = r'/data3/plin/BBA-rsaicp/datasets/rsaicp_608/train/'
    split_path = r'/data3/plin/BBA-rsaicp/datasets/rsaicp_rotate_ms3_auto/train/'
    split = splitbase(origin_path, split_path)
    split.splitdata(1)
    print('1 success!')
    rm_background(split_path)
    ...
    print('0.9 success!')
    rm_background(split_path)
    print('delete 0.9 success!')
    split.splitdata(1.1)
    print('1.1 success!')
    rm_background(split_path)
    print('delete 1.1 success!')
    #split.splitdata(1.2)
    #print('1.2 success!')
    print('split success!')
    # rm background
