...
def mergesingle2(dstpath, nms, fullname):
    name = util.custombasename(fullname)
    #print('name:', name)
    dstname = os.path.join(dstpath, name + '.txt')
    with open(fullname, 'r') as f_in:
        ...
        for splitline in splitlines:
            subname = splitline[0]
            splitname = subname.split('-')
            oriname = splitname[0]
            x, y = int(splitname[1]), int(splitname[2])
...
def mergebase_parallel2(srcpath, dstpath, nms):
    pool = Pool(16)
    filelist = util.GetFileFromThisRootDir(srcpath)

    mergesingle_fn = partial(mergesingle2, dstpath, nms)
    # pdb.set_trace()
    pool.map(mergesingle_fn, filelist)
...
def mergebypoly2(srcpath, dstpath):
    """
    srcpath: result files before merge and nms
    dstpath: result files after merge and nms
    """
